...
try {
    ...
    if (!$check_stmt->fetch()) {
        $msg = 'Event not found or not an assembly event';
        file_put_contents(__DIR__ . '/debug.log', date('Y-m-d H:i:s') . ' - update_event ERROR: ' . $msg . "\n", FILE_APPEND);
    ...
    file_put_contents(__DIR__ . '/debug.log', date('Y-m-d H:i:s') . ' - update_event EXCEPTION: ' . $e->getMessage() . "\n", FILE_APPEND);
